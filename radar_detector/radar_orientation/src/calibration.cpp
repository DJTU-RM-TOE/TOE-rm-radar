...
namespace radar_orientation
{
    ...
    void calibration::keyboardCallback(const radar_interfaces::msg::Keyboard::SharedPtr msg)
    {
        ...
        if (msg->keynum == 101 && point_select > 0)
            point_select--;
        if (msg->keynum == 100 && point[point_select][0] < 1280)
            point[point_select][0]++;
        if (msg->keynum == 97 && point[point_select][0] > 0)
            point[point_select][0]--;
        if (msg->keynum == 115 && point[point_select][1] < 1024)
            point[point_select][1]++;
        if (msg->keynum == 119 && point[point_select][1] > 0)
            point[point_select][1]--;
        if (msg->keynum == 10)
        {
            enter_flag = 1;
